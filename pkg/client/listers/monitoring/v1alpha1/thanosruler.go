...
type ThanosRulerLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha1.ThanosRuler, err error)
...
// List lists all ThanosRulers in the indexer.
func (s *thanosRulerLister) List(selector labels.Selector) (ret []*v1alpha1.ThanosRuler, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.ThanosRuler))
	})
	return ret, err
}
...
type ThanosRulerNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1alpha1.ThanosRuler, error)
...
// List lists all ThanosRulers in the indexer for a given namespace.
func (s thanosRulerNamespaceLister) List(selector labels.Selector) (ret []*v1alpha1.ThanosRuler, err error) {
...
// Get retrieves the ThanosRuler from the indexer for a given namespace and name.
func (s thanosRulerNamespaceLister) Get(name string) (*v1alpha1.ThanosRuler, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("thanosruler"), name)
	}
	return obj.(*v1alpha1.ThanosRuler), nil
}
