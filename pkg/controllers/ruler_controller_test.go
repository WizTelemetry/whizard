...
var _ = Describe("Ruler Controller", func() {
	Context("When reconciling a resource", func() {
		...
		BeforeEach(func() {
			By("creating the custom resource for the Kind Ruler")
			err := k8sClient.Get(ctx, typeNamespacedName, ruler)
			if err != nil && errors.IsNotFound(err) {
				resource := &monitoringv1alpha1.Ruler{
		...
		AfterEach(func() {
			...
			By("Cleanup the specific resource instance Ruler")
			Expect(k8sClient.Delete(ctx, resource)).To(Succeed())
		})
		It("should successfully reconcile the resource", func() {
			By("Reconciling the created resource")
			controllerReconciler := &RulerReconciler{
				Client: k8sClient,
